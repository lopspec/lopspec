...
class IOSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType('LopSpec\IO\IOInterface');
    }
    ...
    function it_will_disable_faking_if_command_line_option_and_config_flag_are_not_set(
        $input,
        $config
    ) {
        $input->getOption('fake')
              ->willReturn(false);
        $config->isFakingEnabled()
               ->willReturn(false);
        $this->isFakingEnabled()
             ->shouldReturn(false);
    }
    ...
    {
        $input->getOption('no-rerun')
              ->willReturn(false);
        $config->isReRunEnabled()
               ->willReturn(true);
        $this->isRerunEnabled()
             ->shouldReturn(true);
    }
    ...
    function it_will_report_bootstrap_path_from_cli_when_different_paths_are_specified_in_config_and_cli(
        ...
        $input->getOption('bootstrap')
              ->willReturn('/path/to/bootstrap.php');
        $config->getBootstrapPath()
               ->willReturn('/path/to/different.php');
        $this->getBootstrapPath()
             ->shouldReturn('/path/to/bootstrap.php');
    }
    ...
    )
    {
        $input->isInteractive()
              ->willReturn(true);
        $input->getOption('no-code-generation')
              ->willReturn(false);
        $input->getOption('stop-on-failure')
              ->willReturn(false);
        $config->isCodeGenerationEnabled()
               ->willReturn(true);
        $config->isStopOnFailureEnabled()
               ->willReturn(false);
