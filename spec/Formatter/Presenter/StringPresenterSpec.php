...
class StringPresenterSpec extends ObjectBehavior
{
    ...
    {
        $this->presentValue('some_string')
             ->shouldReturn('"some_string"');
    }
    function it_presents_simple_type_as_typed_value()
    {
        $this->presentValue(42)
             ->shouldReturn('[integer:42]');
        $this->presentValue(42.0)
             ->shouldReturn('[double:42]');
    }
    ...
    function its_presentValue_displays_invokable_objects_as_objects()
    {
        ...
        $this->presentValue($invokable)
             ->shouldReturn('[obj:LopSpec\Formatter\Presenter\StringPresenter]');
    }
