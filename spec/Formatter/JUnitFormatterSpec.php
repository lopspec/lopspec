...
class JUnitFormatterSpec extends
    ObjectBehavior
{
    function it_aggregates_testcase_nodes_and_store_them_after_specification_run(
        SpecificationEvent $event
    ) {
        $event->getTitle()
              ->willReturn('specification title');
        $event->getTime()
              ->willReturn(42);
        ...
        $this->getTestCaseNodes()
             ->shouldHaveCount(0);
        $this->getExampleStatusCounts()
             ->shouldReturn([
    ...
    function it_aggregates_testsuite_nodes_and_display_them_after_suite_run(
        SuiteEvent $event,
        $io,
        $stats
    ) {
        $event->getTime()
              ->willReturn(48151.62342);
        $stats->getFailedEvents()
              ->willReturn(range(1, 12));
        $stats->getBrokenEvents()
    ...
    function it_stores_a_testcase_node_after_broken_example_run(
        ...
        $this->getTestCaseNodes()
             ->shouldReturn([
            '<testcase name="example title" time="1337" classname="Acme\Foo\Bar" status="broken">'."\n". '<error type="spec\LopSpec\Formatter\ExceptionStub" message="Something went wrong" />'
            . "\n" .
                '<system-err>'."\n".
                    '<![CDATA['."\n".
                        'Exception trace'."\n".
    ...
    function it_stores_a_testcase_node_after_failed_example_run(
        ...
        $this->getTestCaseNodes()
             ->shouldReturn([
            '<testcase name="example title" time="1337" classname="Acme\Foo\Bar" status="failed">'."\n". '<failure type="spec\LopSpec\Formatter\ExceptionStub" message="Something went wrong" />'
            . "\n" .
                '<system-err>'."\n".
                    '<![CDATA['."\n".
                        'Exception trace'."\n".
