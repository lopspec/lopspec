...
namespace spec\LopSpec\Locator;

use LopSpec\Locator\ResourceInterface;
use LopSpec\Locator\ResourceLocatorInterface;
...
class ResourceManagerSpec extends ObjectBehavior
{
    function it_creates_resource_from_classname_using_locator_with_highest_priority(
        ...
        $locator1->supportsClass('Some\Class')
                 ->willReturn(true);
        $locator1->createResource('Some\Class')
                 ->willReturn($resource1);
        $locator2->supportsClass('Some\Class')
                 ->willReturn(true);
        $locator2->createResource('Some\Class')
                 ->willReturn($resource2);
        $this->createResource('Some\Class')
             ->shouldReturn($resource2);
    }
    function it_does_not_allow_two_resources_for_the_same_spec(
        ...
        $this->registerLocator($locator1);
        $this->registerLocator($locator2);
        $resource1->getSpecClassname()
                  ->willReturn('Some\Spec');
        $resource2->getSpecClassname()
                  ->willReturn('Some\Spec');
        $locator1->getAllResources()
                 ->willReturn([$resource1]);
        $locator2->getAllResources()
                 ->willReturn([$resource2]);
    ...
    function it_locates_all_locators_resources_if_query_string_is_empty(
        ...
        $this->registerLocator($locator2);
        $locator1->getAllResources()
                 ->willReturn([$resource3, $resource2]);
        $locator2->getAllResources()
                 ->willReturn([$resource1]);
        ...
        $this->locateResources('')
             ->shouldReturn([$resource1, $resource3, $resource2]);
    }
    function it_locates_resources_using_all_registered_locators(
        ...
        $this->registerLocator($locator1);
        $this->registerLocator($locator2);
        $locator1->supportsQuery('s:query')
                 ->willReturn(true);
        $locator1->findResources('s:query')
                 ->willReturn([$resource3, $resource2]);
        $locator2->supportsQuery('s:query')
                 ->willReturn(true);
        $locator2->findResources('s:query')
                 ->willReturn([$resource1]);

        $resource1->getSpecClassname()->willReturn('Some\Spec1');
        $resource2->getSpecClassname()->willReturn('Some\Spec2');
        $resource3->getSpecClassname()->willReturn('Some\Spec3');
        $this->locateResources('s:query')
             ->shouldReturn([$resource1, $resource3, $resource2]);
    }
    ...
    function it_uses_the_resource_from_the_highest_priority_locator_when_duplicates_occur(
        ...
        $locator2->getAllResources()
                 ->willReturn([$resource2]);
        $this->locateResources('')
             ->shouldReturn([$resource1]);
    }
    function let(
        ResourceLocatorInterface $locator1,
        ResourceLocatorInterface $locator2
    ) {
        $locator1->getPriority()
                 ->willReturn(5);
        $locator2->getPriority()
                 ->willReturn(10);
    }
}
