...
class MethodReturnedNullListenerSpec extends ObjectBehavior
{
    ...
    function it_does_not_prompt_when_there_is_a_problem_creating_the_resource(
        ...
        $resourceManager->createResource(Argument::any())
    ...
    function it_invokes_method_body_generation_when_prompt_is_answered_yes(
        ...
        $this->afterExample($exampleEvent);
        $this->afterSuite($event);
        $generatorManager->generate($resource, 'returnConstant',
            ['method' => 'myMethod', 'expected' => 100])
    ...
    function let(
        ...
        $this->beConstructedWith($io, $resourceManager, $generatorManager,
            $methodAnalyser);
        $exampleEvent->getException()
                     ->willReturn($notEqualException);
        $notEqualException->getActual()
                          ->willReturn(null);
        $notEqualException->getExpected()
                          ->willReturn(100);
        $io->isCodeGenerationEnabled()
           ->willReturn(true);
        $io->askConfirmation(Argument::any())
        ...
        $io->isFakingEnabled()
           ->willReturn(true);
        $methodAnalyser->methodIsEmpty(Argument::cetera())
                       ->willReturn(true);
    }
}
