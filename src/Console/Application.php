    ...
    BaseApplication
{
    ...
    protected function getDefaultInputDefinition()
    {
        ...
        if (array_key_exists('no-interaction', $options)) {
            ...
            $options['no-interaction'] = new InputOption($option->getName(), $option->getShortcut(),
                InputOption::VALUE_NONE, $description);
        }
        $options['config'] = new InputOption('config', 'c', InputOption::VALUE_REQUIRED,
            ...
            if ('extensions' === $key && is_array($val)) {
                foreach ($val as $class) {
                    ...
                    if (!$extension instanceof Extension\ExtensionInterface) {
                        throw new RuntimeException(sprintf('Extension class must implement ExtensionInterface. But `%s` is not.',
                            $class));
                    }
    ...
    {
        $paths = ['phpspec.yml', 'phpspec.yml.dist'];
