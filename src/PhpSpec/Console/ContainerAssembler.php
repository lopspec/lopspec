...
class ContainerAssembler
{
    ...
    public function build(ServiceContainer $container)
    {
        ...
        $this->setupNormalizer($container);
    ...
    protected function setupFormatter(ServiceContainer $container)
    {
        $container->set('formatter.formatters.progress', function (ServiceContainer $c) {
            return new SpecFormatter\ProgressFormatter($c->get('formatter.presenter'), $c->get('console.io'),
                $c->get('event_dispatcher.listeners.stats'));
        });
        $container->set('formatter.formatters.pretty', function (ServiceContainer $c) {
            return new SpecFormatter\PrettyFormatter($c->get('formatter.presenter'), $c->get('console.io'),
                $c->get('event_dispatcher.listeners.stats'));
        });
        $container->set('formatter.formatters.junit', function (ServiceContainer $c) {
            return new SpecFormatter\JUnitFormatter($c->get('formatter.presenter'), $c->get('console.io'),
                $c->get('event_dispatcher.listeners.stats'));
        });
        $container->set('formatter.formatters.dot', function (ServiceContainer $c) {
            return new SpecFormatter\DotFormatter($c->get('formatter.presenter'), $c->get('console.io'),
                $c->get('event_dispatcher.listeners.stats'));
        });
        $container->set('formatter.formatters.html', function (ServiceContainer $c) {
            ...
            return new SpecFormatter\HtmlFormatter($factory, $presenter, $io,
                $c->get('event_dispatcher.listeners.stats'));
        });
    ...
    private function setupConsoleEventDispatcher(ServiceContainer $container)
    {
        $container->setShared('console_event_dispatcher', function (ServiceContainer $c) {
            ...
            array_map(array($dispatcher, 'addSubscriber'), $c->getByPrefix('console_event_dispatcher.listeners'));
    ...
    private function setupEventDispatcher(ServiceContainer $container)
    {
        $container->setShared('event_dispatcher', function (ServiceContainer $c) {
            $dispatcher = new EventDispatcher();
            array_map(array($dispatcher, 'addSubscriber'), $c->getByPrefix('event_dispatcher.listeners'));
        ...
        $container->setShared('event_dispatcher.listeners.class_not_found', function (ServiceContainer $c) {
            return new Listener\ClassNotFoundListener($c->get('console.io'), $c->get('locator.resource_manager'),
        ...
        $container->setShared('event_dispatcher.listeners.named_constructor_not_found', function (ServiceContainer $c) {
            return new Listener\NamedConstructorNotFoundListener($c->get('console.io'),
                $c->get('locator.resource_manager'), $c->get('code_generator'));
        });
        $container->setShared('event_dispatcher.listeners.method_not_found', function (ServiceContainer $c) {
            return new Listener\MethodNotFoundListener($c->get('console.io'), $c->get('locator.resource_manager'),
                $c->get('code_generator'));
        });
        ...
        $container->setShared('event_dispatcher.listeners.method_returned_null', function (ServiceContainer $c) {
            return new Listener\MethodReturnedNullListener($c->get('console.io'), $c->get('locator.resource_manager'),
    ...
    private function setupIO(ServiceContainer $container)
    {
        $container->setShared('console.io', function (ServiceContainer $c) {
            return new IO($c->get('console.input'), $c->get('console.output'), $c->get('console.helper.dialog'),
                new OptionsConfig($c->getParam('stop_on_failure', false), $c->getParam('code_generation', true),
                    $c->getParam('rerun', true), $c->getParam('fake', false), $c->getParam('bootstrap', false)));
        });
    }
    /**
     * @param ServiceContainer $container
    ...
    private function setupLocator(ServiceContainer $container)
    {
        ...
        $container->addConfigurator(function (ServiceContainer $c) {
            ...
            foreach ($suites as $name => $suite) {
                ...
                $srcNS = !empty($suite['namespace']) ? $suite['namespace'] : '';
                $specPrefix = !empty($suite['spec_prefix']) ? $suite['spec_prefix'] : 'spec';
                $srcPath = !empty($suite['src_path']) ? $suite['src_path'] : 'src';
                ...
                $psr4prefix = !empty($suite['psr4_prefix']) ? $suite['psr4_prefix'] : null;
                if (!is_dir($srcPath)) {
                ...
                $c->set(sprintf('locator.locators.%s_suite', $name),
                    function () use ($srcNS, $specPrefix, $srcPath, $specPath, $psr4prefix) {
                        return new Locator\PSR0\PSR0Locator($srcNS, $specPrefix, $srcPath, $specPath, null,
                            $psr4prefix);
                    });
            }
        });
    ...
    private function setupPresenter(ServiceContainer $container)
    {
        ...
        $container->setShared('formatter.presenter.differ', function (ServiceContainer $c) {
            ...
            array_map(array($differ, 'addEngine'), $c->getByPrefix('formatter.presenter.differ.engines'));
            return $differ;
        });
    ...
    private function setupRunner(ServiceContainer $container)
    {
        ...
        $container->set('runner.maintainers.subject', function (ServiceContainer $c) {
            return new Runner\Maintainer\SubjectMaintainer($c->get('formatter.presenter'), $c->get('unwrapper'),
