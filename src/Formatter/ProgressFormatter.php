...
class ProgressFormatter extends ConsoleFormatter
{
    ...
    private function drawStats()
    {
        $io = $this->getIO();
        $stats = $this->getStatisticsCollector();
        $percents = $this->getPercentages($stats->getEventsCount(),
            $stats->getCountsHash());
        $barLengths = $this->getBarLengths($stats->getCountsHash());
        $progress = $this->formatProgressOutput($barLengths, $percents,
            $io->isDecorated());
    ...
    {
        $stats = $this->getStatisticsCollector();
        $specProgress = ($stats->getTotalSpecsCount() == 0) ? 1
            : ($stats->getTotalSpecs()) / $stats->getTotalSpecsCount();
        $targetWidth = ceil($this->getIO()
                                 ->getBlockWidth() * $specProgress);
        asort($counts);
        $barLengths = array_map(function ($count) use ($targetWidth, $counts) {
            return $count ? max(1,
                round($targetWidth * $count / array_sum($counts))) : 0;
        }, $counts);
    ...
    private function updateProgressBar(IO $io, array $progress, $total)
    {
        if ($io->isDecorated()) {
            ...
            $pad = $this->getIO()
                        ->getBlockWidth() - strlen(strip_tags($progressBar));
