...
class StringPresenter implements PresenterInterface
{
    ...
    public function presentValue($value)
    {
        ...
        if ($value instanceof Exception) {
            return $this->presentString(sprintf('[exc:%s("%s")]',
        ...
        switch ($type = strtolower(gettype($value))) {
            ...
            case 'boolean':
                return $this->presentString(sprintf('%s',
                    true === $value ? 'true' : 'false'));
            case 'object':
                return $this->presentString(sprintf('[obj:%s]',
                    get_class($value)));
            case 'array':
                return $this->presentString(sprintf('[array:%d]',
                    count($value)));
            case 'string':
    ...
    protected function presentExceptionStackTrace(Exception $exception)
    {
        ...
        $text .= $this->presentExceptionTraceFunction('throw new '
                                                      . get_class($exception),
        ...
        foreach ($exception->getTrace() as $call) {
            ...
            if (isset($call['class'])) {
                $text .= $this->presentExceptionTraceMethod($call['class'],
                    ...
                    isset($call['args']) ? $call['args'] : []
                );
            } elseif (isset($call['function'])) {
                $text .= $this->presentExceptionTraceFunction($call['function'],
                    isset($call['args']) ? $call['args'] : []
                );
            }
        }
    ...
    {
        $args = array_map([$this, 'presentValue'], $args);
    ...
    protected function presentFileCode($file, $lineno, $context = 6)
    {
        ...
        foreach ($lines as $line) {
            $offset++;
            if ($offset == $lineno) {
                $text .= $this->presentHighlight(sprintf('%4d', $offset) . ' '
                                                 . $line);
            } else {
    ...
    private function shouldSkipTracePresentation(array $call)
    {
        ...
        return isset($call['class'])
               && 0 === strpos($call['class'], "LopSpec\\");
    }
    private function shouldStopTracePresentation(array $call)
    {
        return isset($call['file'])
               && false !== strpos($call['file'], $this->runnerPath);
    }
    /**
     * @type Differ\Differ
     */
     ...
     *
     * This property is used as a constant but PHP does not support setting the
     * value with dirname().
     *
     * @type string
     */
